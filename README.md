...
项目包含一个共享运行配置 **Project Mind (FastAPI + Streamlit)**，一次运行即可同时启动后端和网页。在 PyCharm 顶部的运行配置下拉框中选择它，然后点击运行按钮即可。
...
项目当前在 `app/services/agent.py` 中使用以下系统提示词：

```text
你是项目内的 AI 助手。你有两个只读检索工具：
